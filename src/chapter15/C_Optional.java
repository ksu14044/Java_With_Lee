...
/*
*  === Optional 클래스 ===
* : 값이 존재할 수도 있고 없을 수도 있는 객체를 감싼느 컨테이너, 자바 8에서 추가된 기능
* - null 값을 다룰 때 발생하는 NullPointerException(NPE)를 줄이고
* , 명시적으로 값의 유무를 처리하도록 설계
* */

// 1. Optional 클래스
// : null 이 아닌 값을 표함하거나, 값이 없는 상태를 나타낼 수 있음
...
public class C_Optional {
    public static void main(String[] args) {
        ...
        System.out.println(optional2.orElseGet(() -> "발생시키는 값"));
        try {
            System.out.println(optional2.orElseThrow(() -> new Error("비워질 경우 사용자 정의 예외 발생")));
        } catch (Error e) {
            System.out.println("예외가 발생 했음");
        }
