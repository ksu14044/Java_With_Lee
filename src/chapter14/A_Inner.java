...
*       >> 해당 메서드 내에서만! 사용 가능
*       >> 메서드의 지역 변수에만 접근 가능
*       cf) 로컬 변수(외부 클래스 멤버 변수)가 final인 경우에는 접근 가능
*
* 4. 익명(내부) 클래스
* : 이름이 없는 내부 클래스
...
class OuterClass {
    ...
    class InnerClass {
        void display() {
            System.out.println("외부 클래스의 필드에 접근: " + outerField);

           // System.out.println("외부 클래스의 정적 필드에 접근되나?" + staticField); 접근됨
        }
    }

    // 3. 메서드 내부 클래스 (지역 클래스)
    void outerMethod() {
        String localVar = "메서드 로컬 변수"; // 암묵적으로 final 강조(생략)
        ...
        class MethodClass {
            void display() {
                ...
                // System.out.println("외부 클래스 final에 접근 되나? " + staticField); 접근됨
               // System.out.println("외부 클래스 nonFinal에는 안되겠지 " + outerField);
        ...
        methodClass.display(); // 로컬 변수에 접근(Final X) : 메서드 로컬 변수
...
public class A_Inner {
    public static void main(String[] args) {
        ...
        innerClass.display(); // 외부 클래스의 필드에 접근: 외부 클래스의 필드
        System.out.println();
